...
public class ConnectFourGame extends ListenerAdapter {
    ...
    public ConnectFourGame(User user, User opponent, TextChannel gameChannel) {
        ...
        gameChannel.sendMessage("").addActionRow(buttonsTwo).queue();
    }
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        if (event.getUser().getId().equals(this.playersTurn.getId()) && event.getMessage().getId().equals(this.boardMessage.getId()) && !isCompleted) {
    ...
    private MessageEmbed drawBoard() {
        ...
        return new MessageEmbed(
                null,
                "Connect 4",
                 this.isCompleted ? "<@" + this.winner.getId() + "> Won!"  : "<@" + this.playersTurn.getId() + ">'s turn",
                EmbedType.RICH,
                OffsetDateTime.now(),
                0,
                null,
